...
test.GLM.covtype <- function() {
  ...
  lambdaM <- c(1, 0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001)
  minLogloss1 <- 100
  minLogloss2 <- 100
  trainMetric1 <- NULL
  trainMetric2 <- NULL
  ...
  for (lambda in lambdaM) {
    covtype.h2o1 <- h2o.glm(x=1:54, y=55, training_frame = covtype,
                           family = "multinomial", solver="IRLSM_SPEEDUP", alpha = 0.5, lambda = lambda)
  ...
  print(paste("IRLSM_SPEEDUP best logloss ", minLogloss1))
  print(paste("IRLSM best logloss ", minLogloss2))
