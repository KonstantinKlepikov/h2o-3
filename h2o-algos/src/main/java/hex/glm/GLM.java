...
public class GLM extends ModelBuilder<GLMModel,GLMParameters,GLMOutput> {
  ...
  protected void checkMemoryFootPrint(DataInfo activeData) {
    if (_parms._solver.equals(Solver.IRLSM) || _parms._solver.equals(Solver.COORDINATE_DESCENT) || 
            _parms._solver.equals(Solver.IRLSM_SPEEDUP_TEST) || _parms._solver.equals(Solver.IRLSM_SPEEDUP)) {
      ...
      if ( _parms._solver.equals(Solver.IRLSM_SPEEDUP_TEST) || _parms._solver.equals(Solver.IRLSM_SPEEDUP)) {
    ...
    if(_response != null) {
      ...
      switch (_parms._family) {
        ...
        case multinomial:
          ...
          else if (_parms._solver.equals(Solver.IRLSM_SPEEDUP_TEST) || _parms._solver.equals(Solver.IRLSM_SPEEDUP)) {
            if (!_parms._intercept)
...
//          if (_nclass != 1) error("_family", H2O.technote(2, "Gaussian requires the response to be numeric."));
    ...
    if (expensive) {
      ...
      if (_parms._family.equals(Family.multinomial) && _parms._solver.equals(Solver.IRLSM_SPEEDUP))
  ...
  public final class GLMDriver extends Driver implements ProgressMonitor {
    ...
    private void fitIRLSM_multinomial(Solver s) {
      ...
      } else if (s.equals(Solver.IRLSM_SPEEDUP_TEST) || s.equals(Solver.IRLSM_SPEEDUP)) {
    ...
    private void fitIRLSMSPEEDUP_multinomial(Solver s) {
      ...
      if (s.equals(Solver.IRLSM_SPEEDUP)) { // used only active column set here
...
//      buildModel();
      ...
      if((_parms._family == Family.multinomial || _parms._family == Family.ordinal) && _parms._solver != Solver.L_BFGS ) {
        ...
        if (_parms._family.equals(Family.ordinal))
          _dinfo.addResponse(new String[]{"__glm_ExpC", "__glm_ExpNPC"}, vecs); // store eta for class C and class C-1
                ...
                (_parms._solver.equals(Solver.IRLSM_SPEEDUP)))) {
      ...
      double [] devHistoryTrain = _parms._solver.equals(Solver.IRLSM_SPEEDUP)?new double[30]:new double[5];
      double [] devHistoryTest = _parms._solver.equals(Solver.IRLSM_SPEEDUP)?new double[30]:new double[5];
      ...
      for (int i = 0; i < _parms._lambda.length; ++i) {  // lambda search
        ...
        if (_parms._solver.equals(Solver.IRLSM_SPEEDUP))
          _state.setActiveClass(-1);
        ...
        if(_parms._lambda[i] < _lmax && Double.isNaN(_lambdaCVEstimate) /** if we have cv lambda estimate we should use it, can not stop before reaching it */) {
          if (_parms._early_stopping && _state._iter >= devHistoryTrain.length) {
            double s = _parms._solver.equals(Solver.IRLSM_SPEEDUP)?ArrayUtils.maxAbsValue(devHistoryTrain):ArrayUtils.maxValue(devHistoryTrain);
            if (s < 1e-4) {
...
//      _ginfo = new ProximalGradientInfo(ginfo,ginfo._objVal,ginfo._gradient);
  ...
  public static final class GLMGradientSolver implements GradientSolver {
    ...
    public GLMGradientInfo getGradient(double[] beta) {
      if (_parms._solver.equals(Solver.IRLSM_SPEEDUP_TEST) || _parms._solver.equals(Solver.IRLSM_SPEEDUP)) { // do thing natively here
